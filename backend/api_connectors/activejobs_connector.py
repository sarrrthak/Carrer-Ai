"""
CONNECTOR 2 — Active Jobs DB
Host    : active-jobs-db.p.rapidapi.com
Endpoint: /active-ats-1h
Source  : 175k+ career sites & ATS · hourly refresh
"""
import http.client, json, urllib.parse, hashlib, datetime, re, sys, os
...
# Skill extraction list (shared pattern)
SKILL_LIST = [
...
_SENIOR_RE  = re.compile(r'\b(senior|sr\.?|lead|principal|staff)\b', re.I)
_EXEC_RE    = re.compile(r'\b(director|vp|vice.president|chief|head of|president)\b', re.I)
_ENTRY_RE   = re.compile(r'\b(junior|jr\.?|entry.level|entry|associate|intern|graduate|new.grad)\b', re.I)


def _experience_from_title(title):
    t = title or ""
...
def _extract_skills(description, skills_field):
    """Use API skills field if available, else extract from description."""
...
def raw_call():
    ...
    conn.request("GET",
        '/active-ats-1h?offset=0&title_filter=%22AI%20Engineer%22&description_type=text',
        headers=HEADERS)
...
# ── NORMALIZED fetch ──────────────────────────────────────────────
def fetch_activejobs(title_filter="", location_filter="",
                     offset=0, description_type="text"):
    params = {"offset": offset, "description_type": description_type}
    if title_filter:
    ...
    conn = http.client.HTTPSConnection(HOST)
    conn.request("GET", f"/active-ats-1h?{qs}", headers=HEADERS)
    res  = conn.getresponse()
...
def _normalize(j):
    ...
    loc = _normalize_location(
        j.get("locations_raw") or j.get("location") or j.get("city") or ""
    )
    ...
    # Experience (infer from title)
    title      = j.get("title", "")
    experience = _experience_from_title(title)
    ...
    # is_new: if date_posted absent but date_updated is today, treat as new
    is_new_flag = _is_new(date_str)
    if not date_str:
        updated = (j.get("date_updated") or "")[:10]
        is_new_flag = _is_new(updated)

    # ID
    uid = str(j.get("id") or j.get("url") or (title + (j.get("organization") or j.get("company") or "")))
...
if __name__ == "__main__":
    print("=" * 56)
    print("  CONNECTOR 2 — Active Jobs DB")
    print("  Host:", HOST)
    print("=" * 56)
    out = raw_call()
